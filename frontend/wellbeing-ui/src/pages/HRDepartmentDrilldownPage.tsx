...
function getComparisonClass(value: number, positiveIsGood: boolean) {
...
function HRDepartmentDrilldownPage() {
    ...
    async function handleExportDepartmentPdf() {
        try {
            setIsExporting(true);

            await exportDepartmentReportPdf(
                decodedDepartmentName,
                selectedDays
            );
        } finally {
                ...
                <section className="rounded-3xl border border-white/70 bg-white/90 p-6 shadow-sm backdrop-blur">
                    <div className="mb-2 flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                        ...
                        <button
                            ...
                            className="w-fit rounded-xl bg-[#4caf58] px-4 py-2 text-sm font-semibold text-white shadow-lg shadow-green-500/20 transition hover:bg-[#43a04f] disabled:cursor-not-allowed disabled:opacity-60"
                        >
                            {isExporting ? "Exporting..." : "Export Department PDF"}
                        </button>
                    </div>
                    ...
                    {data && (
                        <section className="mt-8 rounded-3xl border border-white/70 bg-white/90 p-6 shadow-sm backdrop-blur">
                            <div className="mb-6 flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
                                <div>
                                    ...
                                    <p className="mt-1 text-sm text-slate-500">
                                        Compare {data.department} against the company average for the selected period
                                    </p>
                                </div>

                                <span
                                    className={`inline-flex w-fit rounded-full px-3 py-1 text-sm font-semibold ${data.riskDifference < 0
                                            ? "bg-emerald-100 text-emerald-700"
                                            : data.riskDifference > 0
                                ...
                                >
                                    {data.riskDifference < 0
                                        ...
                                        : data.riskDifference > 0
                                            ? "⚠ Higher risk than company average"
                                            : "Equal to company average"}
                                </span>
                            </div>
                            ...
                            <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
                                <ComparisonCard
                                    label="Stress"
                                    departmentValue={data.averageStress}
                                    companyValue={data.companyAverageStress}
                                    difference={data.stressDifference}
                                ...
                                <ComparisonCard
                                    label="Energy"
                                    departmentValue={data.averageEnergy}
                                    companyValue={data.companyAverageEnergy}
                                    difference={data.energyDifference}
                                    positiveIsGood={true}
                                ...
                                <ComparisonCard
                                    label="Risk Score"
                                    departmentValue={data.riskScore}
                                    companyValue={data.companyRiskScore}
                                    difference={data.riskDifference}
                                    positiveIsGood={false}
            ...
            <div className="space-y-2">
                <div className="flex items-center justify-between text-sm">
                    ...
                    <span className="font-semibold text-slate-900">
                        {departmentValue.toFixed(1)}
                    </span>
                </div>

                <div className="flex items-center justify-between text-sm">
                    <span className="text-slate-500">Company avg.</span>
                    <span className="font-semibold text-slate-900">
                ...
                <div className="border-t border-slate-200 pt-2">
                    <p
                        className={`text-sm font-semibold ${getComparisonClass(
                            difference,
                            positiveIsGood
                        )}`}
                    >
                        {formatDifference(difference)} vs company
                    </p>
                </div>
            </div>
        </div>
